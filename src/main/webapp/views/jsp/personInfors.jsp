<%-- <%@page import="com.quicksure.insurance.entity.InsuranceDetailsVO"%> --%>
...
<html lang="en">
 ...
 <div class="background_img" style='background:-webkit-radial-gradient(#fdfdfd 60%,#f1f1f1);background:radial-gradient(#fdfdfd 60%,#f1f1f1);'>
  <div class="infors_content" id="infor_content">
     <form class="content_box lf" id="submit_form" action="<%=path%>/submitInfor/submitUnderwriting.do" method="post">
        ...
        <input type="text" style="display: none;" id="deliveryCopyCheckbox" name="deliveryCopyCheckbox" value="${insuranceDetailsVO.insuranceperinfor.deliveryCopyCheckbox}" readonly="readonly"/>
         ...
         <div class="car_infor">
             <ul>
                 <li><span class="title_lis">车牌号码:</span><span>${insuranceDetailsVO.vhlinfor.lcnno}</span></li>
                 <li><span class="title_lis">发动机号:</span><span>${insuranceDetailsVO.vhlinfor.engno}</span></li>
                 <li><span class="title_lis">品牌型号:</span><span>${insuranceDetailsVO.vhlinfor.model}</span></li>
                 <li><span class="title_lis">车架号VIN:</span><span>${insuranceDetailsVO.vhlinfor.vinno}</span></li>
             </ul>
         </div>
         <div class="insure_content">
             ...
             <table class="table_2">
                 <c:if test="${!empty insuranceDetailsVO.baseinfor.jqpremium}"><tr class="tr_list"><td>交强险</td><td></td><td><span>${insuranceDetailsVO.baseinfor.jqpolicystartdate}</span>起</td><td></td><td>&yen;${insuranceDetailsVO.baseinfor.jqpremium}</td></tr></c:if>
                 <c:if test="${!empty insuranceDetailsVO.baseinfor.sypremium}"><tr class="tr_list"> <td>商业险</td><td></td><td><span>${insuranceDetailsVO.baseinfor.sypolicystartdate}</span>起</td> <td></td><td>&yen;${insuranceDetailsVO.baseinfor.sypremium}</td></tr></c:if>                         
                 <c:if test="${!empty insuranceDetailsVO.baseinfor.taxpremium}"><tr class="tr_list"><td>车船税</td><td></td><td></td><td></td> <td>&yen;${insuranceDetailsVO.baseinfor.taxpremium}</td></tr></c:if>
                
             </table>
             <p class="total_cost" style="padding-right: 40px;">保费总额:<span>&yen;<fmt:formatNumber value="${insuranceDetailsVO.baseinfor.totalPremium + 0.0001}" pattern="#,##0.00"/></span>元</p>
             <div class="table_3" style="margin-top: 20px;">
                 <table class="insure_titop">
                     <tr class="tb3_title">
                         <td>险种明细</td>
                         <td>保额( &yen; )</td>
                         <td>保费( &yen; )</td>
                 ...
                 <table style="width:738px;margin-top: 15px;">
                 <c:forEach items="${insuranceDetailsVO.coverageinfors}" var="s">
                     <c:if test="${s.insrnccode=='0357'}">
                     	<tr class="tb3_list">
                        	<td>交强险</td>
	                        <td><span class="rg insure_price">
	                        	<c:if test="${empty s.suminsured || s.suminsured==''|| s.suminsured=='0' || s.suminsured=='0.00'}">--</c:if>
	                        	<c:if test="${!empty s.suminsured && s.suminsured!=''&& s.suminsured!='0' && s.suminsured!='0.00'}">${s.suminsured}</c:if>
	                        </span></td>
	                        <td><span class="rg rebill">
	                        	<c:if test="${empty s.premium || s.premium==''|| s.premium=='0' || s.premium=='0.00'}">--</c:if>
                    			<c:if test="${!empty s.premium && s.premium!=''&& s.premium!='0' && s.premium!='0.00'}">${s.premium}</c:if>
	                        </span></td>
	                        <td><span class="rg no_dedu">
		                        <c:if test="${empty s.nyl12 || s.nyl12==''|| s.nyl12=='0' || s.nyl12=='0.00'}">--</c:if>
	                    		<c:if test="${!empty s.nyl12 && s.nyl12!=''&& s.nyl12!='0' && s.nyl12!='0.00'}">${s.nyl12}</c:if>
	                    	</span></td>
                     	</tr>
                     </c:if>
                     <c:if test="${s.insrnccode=='030101'}">
	                 	 <tr class="tb3_list">
	                         <td>车辆损失险</td>
	                         <td><span class="rg insure_price">
		                         <c:if test="${empty s.suminsured || s.suminsured==''|| s.suminsured=='0' || s.suminsured=='0.00'}">--</c:if>
         ...
         <div class="insurance_tab_list">
             <div class="tab_list"><span class="car_owner active" style='font-size:19px;padding-bottom:1px;'>车主信息</span><span class="bbr"><strong>被保人</strong><i data-val='1'></i>同车主</span><span class="tbr"><strong>投保人</strong><i data-val='2'></i>同车主</span><b></b></div>
             <div class="main_infors">
                 <table id="car_owner">
                     <tr>
	                     <td><span>车主姓名</span><input type="text" value="${insuranceDetailsVO.vhlinfor.drvowner}" name="ownersname" id="ownersname" maxlength="4" onchange="changevalue('ownersname');"></td>
	                     <td><span>车主身份证</span><input type="text" value="${insuranceDetailsVO.vhlinfor.certificateno}" name="ownerscerticode" id="ownerscerticode" maxlength="18" onchange="changevalue('ownerscerticode');" onblur="setUpperCase(this);"  onkeyup="if(value!=value.replace(/[\W]/g,''))value=value.replace(/[\W]/g,'')"></td>
                     </tr>
                     <tr>
	                     <td><span>手机号</span><input type="text" value="${insuranceDetailsVO.vhlinfor.phoneno}" name="ownersphoneno" id="ownersphoneno" maxlength="11" onchange="changevalue('ownersphoneno');"  onKeyUp="if(value!=value.replace(/[^\d]/g,''))value=value.replace(/[^\d]/g,'')"></td>
	                     <td></td>
                     </tr>
                     <tr>
	                     <td> <span>地址</span><input id="city" name="deptAddress"   type="text" class="city" value="${insuranceDetailsVO.baseinfor.deptAddress}" onblur="setDeptNo(this.value,1)" onchange="changevalue('ownersaddress');" placeholder="请选择对应的城市" readonly="readonly"></td>
	                     <td><span>详细地址</span><input type="text"  id="ownersdetailaddress" required ='required' name="ownersdetailaddress" onchange="changevalue('ownersdetailaddress');" value="${insuranceDetailsVO.insuranceperinfor.ownersdetailaddress}"></td>
	                     <input type="hidden" id="ownerszipcode" name="ownerszipcode">
                     </tr>
                 </table>
                 <table id="bbr">
                     <tr>
                         <td><span>被保人姓名</span><input type="text" value="${insuranceDetailsVO.insuranceperinfor.applicationname}" placeholder="请输入被保人姓名" required ='required' maxlength="4" name="applicationname" id="applicationname" ></td>
                         <td><span>被保人身份证</span><input type="text" value="${insuranceDetailsVO.insuranceperinfor.applicationcerticode}" placeholder="请输入被保人身份证号码" 
       							required ='required' name="applicationcerticode" id="applicationcerticode" maxlength="18"  onblur="setUpperCase(this);" 
        						onkeyup="if(value!=value.replace(/[\W]/g,''))value=value.replace(/[\W]/g,'')"></td>
                     </tr>
                     <tr>
                         <td><span>手机号</span><input type="text" value="${insuranceDetailsVO.insuranceperinfor.applicationphoneno}" placeholder="请输入被保人手机号码" required ='required'name="applicationphoneno" id="applicationphoneno" maxlength="11" 
        						 onKeyUp="if(value!=value.replace(/[^\d]/g,''))value=value.replace(/[^\d]/g,'')"></td>
                         <td></td>
                     </tr>
                     <tr>
                         <td> <span>地址</span><input  id="city2" type="text" class="city" name="applicationaddress" value="${insuranceDetailsVO.insuranceperinfor.applicationaddress}" placeholder="请选择对应的城市" readonly="readonly" ></td>
                         <td><span>详细地址</span><input type="text" name="applicationdetailaddress" id="applicationdetailaddress"  value="${insuranceDetailsVO.insuranceperinfor.applicationdetailaddress}"></td>
                         <input type="hidden" id="applicationzipcode" name="applicationzipcode">
                     </tr>
                 </table>
                 <table id="tbr">
                     <tr>
                         <td><span>投保人姓名</span><input type="text" required ='required'type="text" value="${insuranceDetailsVO.insuranceperinfor.insurename}" placeholder="请输入投保人姓名" name="insurename" id="insurename" maxlength="4"></td>
                         <td><span>投保人身份证</span><input type="text"  required ='required'type="text" value="${insuranceDetailsVO.insuranceperinfor.insurecerticode}" placeholder="请输入投保人身份证号码" 
       						name="insurecerticode" id="insurecerticode"  maxlength="18" onblur="setUpperCase(this);" 
       						onkeyup="if(value!=value.replace(/[\W]/g,''))value=value.replace(/[\W]/g,'')"></td>
                     </tr>
                     <tr>
                         <td><span>手机号</span><input type="text"  required ='required'type="text" value="${insuranceDetailsVO.insuranceperinfor.insurephoneno}" placeholder="请输入投保人手机号码" name="insurephoneno" id="insurephoneno" maxlength="11" 
        					onKeyUp="if(value!=value.replace(/[^\d]/g,''))value=value.replace(/[^\d]/g,'')"></td>
                         <td></td>
                     </tr>
                     <tr>
                         <td><span>地址</span><input type="text"  id="city3"  class="city" value="${insuranceDetailsVO.insuranceperinfor.insureaddress}" placeholder="请选择对应的城市" readonly="readonly" name="insureaddress"></td>
                         <td><span>详细地址</span><input type="text"  placeholder="*必填——详细地址" required ='required' id="insuredetailaddress"  name="insuredetailaddress" value="${insuranceDetailsVO.insuranceperinfor.insuredetailaddress}"></td>
                         <input type="hidden" id="insurezipcode" name="insurezipcode">
                     </tr>
                 </table>
             </div>
             <div class="head_title ">
                 <i></i><span>送单地址</span><p class='get_address_top'><i></i>同车主</p>
             </div>
             <table class="get_single_address">
                 <tr>
                     <td><span>收件人姓名</span><input type="text" required ='required' id="deliveryname" name="deliveryname" placeholder="请输入收件人姓名" maxlength="4" value="${insuranceDetailsVO.deliveryinfor.deliveryname}"></td>
                     <td><span>手机号</span><input type="text" required ='required' id="deliveryphone" name="deliveryphone" placeholder="请输入收件人手机号"  maxlength="11" value="${insuranceDetailsVO.deliveryinfor.deliveryphone}" onKeyUp="if(value!=value.replace(/[^\d]/g,''))value=value.replace(/[^\d]/g,'')"></td>
                 </tr>
                 <tr>
                     <td><span>地址</span><input type="text" class="city" id="city4" name="deliveryaddress" placeholder="请选择对应的城市" value="${insuranceDetailsVO.deliveryinfor.deliveryaddress}"></td>
                     <td><span>详细地址</span><input type="text" placeholder="*必填——详细地址" required ='required' id="deliverydetailaddress" name="deliverydetailaddress" value="${insuranceDetailsVO.insuranceperinfor.deliverydetailaddress}"></td>
                     <input type="hidden" id="Delivery_ZipCode" name="Delivery_ZipCode">
                 </tr>
             </table>
             <p class="stipulation"><label  class='check'><input  type="checkbox"></label> 我已阅读并同意 <span class="LudiDCDN_content">《免责说明》</span> <span class="LudiDCInClause_content">《保险条款》</span> <span class="LudiDCCTPLClause">《强险条款》</span> <span class="LudiDCInDetion_content">《投保说明》</span> 条款</p>
         </div>
         <div class="submit "><a id="next_p">下一步</a></div>
	</form>
...
<div class='mask_layer'>
   <div class='stipulation_div'>
     <div class="stipulation_title"></div>
      <div id='stipulation_box' style="height: 450px;width: 100%;">
          <div id="stipulation_content" style="height: 1000px;width:96%;margin: 0px auto;top: 0px;position: absolute;"></div>
      </div>
   </div>
</div>
 ...
 <script  src="<%=path %>/views/js/plugin/optiscroll.js "></script>
 <script type="text/javascript">
	$(document).ready(function(){
	     $('#head_top').load('<%=path%>/views/jsp/header.jsp');
         $('#footer').load('<%=path%>/views/jsp/footer.jsp');
	});
var os1 = new Optiscroll(document.getElementById('stipulation_box'), { preventParentScroll: true });
  
    $('.stipulation span').on('click',function(){
            var Id="#"+$(this).attr('class'),Title=$(this).text();
            $('.stipulation_title').html(Title);
            $('#stipulation_content').html($(Id).html());
            $('.mask_layer').show();
            return false;
    })
...
$('.stipulation_div').on('click', function () {
	return false;
});
</script>
<script type="text/javascript">
	...
	if (error == errorCode || error2 == errorCode) {
		if(agentFlag == "1"){
			...
			$("#Message").html("提交失败，请拨打客服热线10100111-6咨询");	
			$("#errorhei").show();
		}
	}
</script>
