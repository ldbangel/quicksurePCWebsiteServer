...
//	        		checkphoneNoFlag=true;
	        		if(checkphoneNoFlag){
	        			...
	        			if(checkpswFlag&&checkconfirmFlag){
	        				var num=60;
	        				var Span=document.getElementById('CheckCode').getElementsByTagName('span')[0];
	    									   ...
	    									   if(result=="true"){//手机验证码校验成功
	    								            ...
	    								            success : function(result){
	    								            	...
	    								            	if(result=="1"){
	    								            		...
	    								            		clearInterval(_interval);
	    													$('#CheckCode span').eq(4).html('获取').removeClass('Color');							            		
	    								            			  $("#Message").html("用户注册成功.");
	    									            		  $(".errorhei").show();
	    								            		   setTimeout(function(){
	    								            			  //$('.errorhei').hide();
	    								            			  window.location.href = getUrl()+"/views/jsp/loginUser.jsp?address="+address+"&chepai="+chepai;
	    								            			},1000);
	...
	$.ajax({
        ...
        success : function(result){
    		...
    		if("绑定成功"===result){
        		...
        		$(".errorhei").show();
        		//Hide();
    			window.location.href = getUrl()+"/views/jsp/quickSurehome.jsp";
    		}
...
/*$("#rlogin").click(function(){
	var userName=$('.removea ').text();
	if(userName == '登录'){
		login();
	}else{
		$("#Message").html("账户已登录，请注销后重新登录！");
		$(".errorhei").show();
	}
...
//登录
function login(){
	window.location.href=getUrl()+"/views/jsp/loginUser.jsp?address="+address+"&chepai="+chepai+"&dptcode="+dptcode;
}